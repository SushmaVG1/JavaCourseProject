...
public class AddPhotography extends JFrame {
	...
	public AddPhotography() {
		...
		JLabel lblNewLabel = new JLabel("Enter the photography service details to be added");
		lblNewLabel.setFont(new Font("Tahoma", Font.PLAIN, 18));
		lblNewLabel.setBounds(391, 62, 439, 52);
		...
		JButton btnNewButton = new JButton("Add photography service");
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (!name.equals("") && !address.equals("") && !phNo.equals("") && !email.equals("")) {
					try {
						...
						if (charges < 2000)
							JOptionPane.showMessageDialog(null, "Minimum charges has to be Rs. 2000");
						else {
							Photography p = new Photography(name, address, phNo, email, charges);
							Main.arrayPhotography.add(p);
							try {
								Class.forName("oracle.jdbc.driver.OracleDriver");
								Connection con = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe",
										"SYSTEM", "123");
								Statement stmt = con.createStatement();
								ResultSet rs = stmt.executeQuery("select * from photography");
								String query = "insert into photography values('" + name + "', '" + address + "' , '"
										+ phNo + "','" + email + "', " + charges + ")";
								stmt.executeUpdate(query);
								JOptionPane.showMessageDialog(null, "Photography service added");
								con.close();
							} catch (Exception ep) {
		...
		btnNewButton.setBounds(485, 548, 191, 25);
